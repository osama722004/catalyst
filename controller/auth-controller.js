...
const register = async (req, res, next) => {
    ...
    if (!isValidPassword(password)) {
        return res.status(400).send('Password must be at least 6 characters long and contain characters, numbers, and symbols');
    }
...
const login = async (req, res, next) => {
    ...
    if(!email || !password) {
        const error = appError.create('email or password are required', 400, "fail")
        return next(error);
    }
